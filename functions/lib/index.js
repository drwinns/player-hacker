...
const dialogflow_helper_1 = require("dialogflow-helper");
dialogflow_helper_1.init({
...
exports.webhook = firebase_functions_1.https.onRequest(async (request, response) => {
    try {
        const _agent = new dialogflow_fulfillment_1.WebhookClient({ request, response });
        ...
        //     });
        // }
        async function productForHealthProblem(agent) {
            ...
            if (!HealthProblem) {
                ...
                await dialogflow_helper_1.agent.getEntity("fc689e87-a9fc-4749-8d81-ee1dff6583c8").then((entity) => {
                    console.log("received entity: ", entity.name);
                    const entities = entity.entities;
                    conv.ask(new actions_on_google_1.Suggestions(pluck(entities).synonyms[0], pluck(entities).synonyms[0], pluck(entities).synonyms[0]));
                    agent.add(conv);
                    return;
                }).catch(e => {
                ...
                await Promise.all([
                    axios_1.default.get("https://youngofficial.com/wp-json/wc/v3/products?per_page=100&page=1", {
                        headers: {
                    ...
                    axios_1.default.get("https://youngofficial.com/wp-json/wc/v3/products?per_page=100&page=2", {
                        headers: {
                            'Authorization': 'Basic Y2tfOTdkNGZmZTJhMmEwNTFhNTBiNmQyY2ZmNWQzOTA2ZDE5MGU3ZjhiODpjc185ZTA2Yjg2YzIxYzJmOTA4YjQ2NTI2NjA2NDUxYTNiOTlhMzQ3ODc1',
                            'Cookie': 'guest_user=d3d1d697d983ff5d905ddcd3d9bac3e5'
                        }
                    ...
                    products.map((eachProduct) => {
                        ...
                        eachProduct.attributes.map((eachAttribute) => {
                    ...
                    if (items.length) {
                        ...
                        conv.ask(new actions_on_google_1.BrowseCarousel({ items: items }));
                        ...
                        conv.ask(new actions_on_google_1.BasicCard({
                            buttons: [
                                new actions_on_google_1.Button({ title: `Wallach on ${HealthProblem} (3rd Party Site)`, url: 'https://app.searchie.io/widget/yJe1BjV1ak#/search/' + HealthProblem })
                            ],
                            title: 'Search Wallach Database',
                            image: new actions_on_google_1.Image({
                                url: 'https://youngofficial.com/wp-content/uploads/2019/06/DrWallach-circle.png',
        ...
        async function getProductInfo(agent) {
            ...
            await Promise.all([
                ...
                if (!productName) {
                    ...
                    conv.ask(new actions_on_google_1.Suggestions(pluck(products).name));
                    conv.ask(new actions_on_google_1.Suggestions(pluck(products).name));
                    conv.ask(new actions_on_google_1.Suggestions(pluck(products).name));
                    agent.add(conv);
                    return;
                    ...
                    products.map((eachProduct) => {
                        ...
                        if (parseInt(eachProduct.id) === productName) {
                            ...
                            conv.ask(new actions_on_google_1.BasicCard({
                                ...
                                buttons: [
                                    new actions_on_google_1.Button({ title: "Learn More", url: eachProduct.permalink }),
                                ],
                            }));
                            conv.ask(new actions_on_google_1.LinkOutSuggestion({
        ...
        function CaptureUserInfo(agent) {
            console.log("agent.parameters: ", agent.parameters);
            const { firstname, lastname, email, mobilephone } = agent.parameters;
            const data = [{
                    firstname: firstname,
                    lastname: lastname,
                    Email: email,
            ...
            axios_1.default.post('https://sheet.best/api/sheets/36020baa-fccb-4667-af52-90759d44e976', data).catch(e => {
        ...
        function welcome(agent) {
            ...
            conv.ask(new actions_on_google_1.Suggestions("Show me products"));
            conv.ask(new actions_on_google_1.Suggestions("Dr. Wallach Info"));
            conv.ask(new actions_on_google_1.Suggestions("90 Essentials"));
            agent.add(conv);
            return;
        }
